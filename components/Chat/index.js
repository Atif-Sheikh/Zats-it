...
class ChatRoom extends Component {
    ...
    onPress = () => {
        const { route: { params: { number } } } = this.props;
                            ...
                            <View key={ind}>
                                <View
                                    ...
                                    <Text
                                        style={{ color: "#a10000" }}
                                    >{val.msg}</Text>
                                    <Text style={{ color: "#a10000" }} note>{moment(val.time).fromNow()}</Text>
                                </View>
                            </View>
                        )
                    })}
                    {!this.state.messages.length ? <Text style={{ alignSelf: 'center', justifyContent: 'center', color: 'grey', marginTop: 30 }}>No Message</Text> : null}
                </ScrollView>
